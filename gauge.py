...
class DeepGauge(object):
    ...
    def init_bounds(self):
        ...
        for layer in self.layer_to_compute:
            if os.path.exists(
                    '/cache/{model_name}/train_outputs/'
    ...
    def init_knc_coverage(self, x):
        ...
        :return:
        """
        if os.path.exists('/cache/{model_name}/test_outputs/'
                          'knc_coverage.pkl'.format(model_name=self.model_name)):
            with open('/cache/{model_name}/test_outputs/'
            ...
            with open('/cache/{model_name}/test_outputs/'
                      'knc_coverage.pkl'.format(model_name=self.model_name), 'wb') as f:
                ...
                for output in layer_outputs:
                    for id_neuron in range(output.shape[-1]):
                        ...
                        if val < low_bound[id_neuron] and not self.nbc_cov_dict[layer.name][id_neuron][0]:
                            ...
                            continue
            with open('/cache/{model_name}/test_outputs/'
                      'nbc_coverage.pkl'.format(model_name=self.model_name), 'wb') as f:
